...
setwd("D:/Users/Yanxi Fang/Files/Fall 2020/Gov 1347/Blog Post Work/Week 8/Data from Previous Weeks")
...
outsamp_errors <- sapply(1:1000,function(i){
  years_outsamp <- sample(poll_avg_st_plusecon_short$year,3)
  outsamp_mod <- lm(candidate_pv ~ avg_poll + avg_delta_inc,
          poll_avg_st_plusecon_short[!(poll_avg_st_plusecon_short$year %in% years_outsamp),])
  outsamp_pred <- predict(outsamp_mod, newdata=poll_avg_st_plusecon_short[poll_avg_st_plusecon_short$year %in% years_outsamp,])
  outsamp_true <- poll_avg_st_plusecon_short$candidate_pv[poll_avg_st_plusecon_short$year %in% years_outsamp,]
