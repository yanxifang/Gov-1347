...
plot_usmap(data=HB_new_2, regions="counties", values="winner") +
  scale_fill_manual(values=c("cornflowerblue","firebrick2"),na.value="gray",
                    name="Predicted Winner of County")+
  theme_void()

plot_usmap(data=HB_orig_2, regions="counties", values="winner") +
  scale_fill_manual(values=c("cornflowerblue","firebrick2"),na.value="gray",
                    name="Predicted Winner of County")+
  theme_void()
# very inaccurate graphs: e.g. MA shows as all Republican!

# new approach: filter out counties without COVID data
demog_change_2 <- demog_change %>%
  filter(d_pv!="NA")
demog_change_2 <- demog_change_2 %>%
  filter(deaths!="NA")
county_d_lm_6 <- lm(d_pv ~ Asian_change+Asian_change*Asian+
                      Black_change+Black_change*Black+
                      Hispanic_change+Hispanic_change*Hispanic+
                      Indigenous_change+Indigenous_change*Indigenous+
                      Female_change+Female_change*Female+
                      ...
                      age4565_change+age4565_change*age4565, 
                    demog_change_2)
summary(county_d_lm_6)
predict(county_d_lm_6, newdata=demog_2020_change) +
  (0.768874-0.384437)*demog_2020_change$Black +
  (0.117700-0.058850)*demog_2020_change$Black_change +
  (0.811650-0.405825)*demog_2020_change$Hispanic +
  (-2.70348+1.351740)*demog_2020_change$Hispanic_change

HB_orig_3 <- data.frame(pred = predict(county_d_lm_6, newdata = demog_2020_change),
                        state = demog_2020_change$state, 
                        county = demog_2020_change$fips_county)
HB_new_3  <- data.frame(pred = predict(county_d_lm_6, newdata = demog_2020_change) +
                          (0.768874-0.384437)*demog_2020_change$Black +
                          (0.117700-0.058850)*demog_2020_change$Black_change +
                          (0.811650-0.405825)*demog_2020_change$Hispanic +
                          (-2.70348+1.351740)*demog_2020_change$Hispanic_change,
...
table(HB_orig_3$winner)
table(HB_new_3$winner)

plot_usmap(data=HB_new_3, regions="counties", values="winner") +
  scale_fill_manual(values=c("cornflowerblue","firebrick2"),na.value="gray",
                    name="Predicted Winner of County")+
  theme_void()
#still getting unexpected results

# now, no longer changing the interaction term coefficient
county_d_lm_7 <- lm(d_pv ~ Asian_change+Asian_change*Asian+
                      Black_change+Black_change*Black+
                      Hispanic_change+Hispanic_change*Hispanic+
                      Indigenous_change+Indigenous_change*Indigenous+
                      Female_change+Female_change*Female+
                      ...
                      age4565_change+age4565_change*age4565, 
                    demog_change_2)
summary(county_d_lm_7)
predict(county_d_lm_7, newdata=demog_2020_change) +
  (0.768874-0.384437)*demog_2020_change$Black +
  (0.811650-0.405825)*demog_2020_change$Hispanic

HB_orig_4 <- data.frame(pred = predict(county_d_lm_7, newdata = demog_2020_change),
                        state = demog_2020_change$state, 
                        county = demog_2020_change$fips_county)
HB_new_4  <- data.frame(pred = predict(county_d_lm_7, newdata = demog_2020_change) +
                          (0.768874-0.384437)*demog_2020_change$Black +
                          (0.811650-0.405825)*demog_2020_change$Hispanic,
                        state = demog_2020_change$state,
                        county = demog_2020_change$fips_county)
...
plot_usmap(data=HB_new_4, regions="counties", values="winner") +
  scale_fill_manual(values=c("cornflowerblue","firebrick2"),na.value="gray",
                    name="Predicted Winner of County")+
  labs(title="Predicted Winner by County",subtitle="Based on Double Black and Latinx Turnout") +
  theme_void()
ggsave("pred_winner_double_turnout.png", height = 4, width = 8)
